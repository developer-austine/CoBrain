...
/**
 * Google Drive OAuth + sync control, matching the Gmail/Notion nodes.
 *
 ...
 */
export default function DriveConnectButton({ param, nodeId, updateNodeParamValue }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/drive/status?workflowId=${workflowId}&nodeId=${nodeId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleConnect = () => {
    ...
    const url = `/api/drive/auth?nodeId=${nodeId}&workflowId=${workflowId}`;
    const popup = window.open(url, "driveOAuth", "width=520,height=720,left=200,top=80");
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setSyncMsg(`${data.synced} files synced`);
      setStatus("connected");
    } catch (err) {
